...
from barter.models import Invoice
...
class PaymentViewTests(TestCase):
    ...
    def test_view_payment_status_code(self):
        response = self.client.get(reverse("barter:purchase-summary", kwargs={'uuid': Invoice.objects.get(pk=1).uuid}))
    ...
    def test_view_payment_status_code_fail_no_login(self):
        client = Client()
        response = client.get(reverse("barter:purchase-summary", kwargs={'uuid': Invoice.objects.get(pk=1).uuid}))
