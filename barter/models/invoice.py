...
class Invoice(CreateUpdateModelBase):
    ...
    #     """
    #     return reverse('barter:invoice-detail', kwargs={'uuid': self.uuid})

    def get_absolute_management_url(self):
        """
        This is the url to the management's detail page for the Invoice
        """
        return reverse('barter_admin:manager-order-detail', kwargs={'uuid': self.uuid})
...
class OrderItem(CreateUpdateModelBase):
    ...
    '''
    invoice = models.ForeignKey("barter.Invoice", verbose_name=_("Invoice"), on_delete=models.CASCADE, related_name="order_items")
    offer = models.ForeignKey("barter.Offer", verbose_name=_("Offer"), on_delete=models.CASCADE, related_name="order_items")
