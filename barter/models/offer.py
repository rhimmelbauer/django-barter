...
from barter.config import BARTER_PRODUCT_MODEL, DEFAULT_CURRENCY
...
class Offer(CreateUpdateModelBase):
    '''
    Offer attaches to a record from the designated BARTER_PRODUCT_MODEL.  
    ...
    def add_to_cart_link(self):
        return reverse("barter:add-to-cart", kwargs={"slug":self.slug})

    def remove_from_cart_link(self):
        return reverse("barter:remove-from-cart", kwargs={"slug":self.slug})
