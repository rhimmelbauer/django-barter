...
from barter import config
from barter.models import Payment, Invoice, Receipt, Address
from barter.models.choice import PurchaseStatus, TermType
##########
# SIGNALS

barter_pre_authorization = django.dispatch.Signal()
barter_process_payment =  django.dispatch.Signal()
barter_post_authorization =  django.dispatch.Signal()
...
class PaymentProcessorBase(object):
    ...
    def set_api_endpoint(self):
        """
        Sets the API endpoint for debugging or production.It is dependent on the BARTER_STATE
        enviornment variable. Default value is DEBUG for the BARTER_STATE this function
        should be overwrote upon necesity of each Payment Processor
        """
        if config.BARTER_STATE == 'DEBUG':
            self.API_ENDPOINT = None
        elif config.BARTER_STATE == 'PRODUCTION':
            self.API_ENDPOINT = None
    ...
    def authorize_payment(self):
        ...
        barter_pre_authorization.send(sender=self.__class__, invoice=self.invoice)
        ...
        barter_process_payment.send(sender=self.__class__, invoice=self.invoice)
        ...
        barter_post_authorization.send(sender=self.__class__, invoice=self.invoice)
